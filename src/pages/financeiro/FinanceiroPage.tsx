...
const FinanceiroPage: React.FC = () => {
  ...
  const handleGerarRelatorioPDFPeriodo = () => {
    ...
    const contasReceberPeriodo = contasReceber.filter(conta => {
      const d = new Date(`${conta.data_vencimento}T00:00:00`);
      return d >= inicio && d <= fim;
    });
    const contasPagarPeriodo = contasPagar.filter(conta => {
      const d = new Date(`${conta.data_vencimento}T00:00:00`);
      return d >= inicio && d <= fim;
    });
    const totalReceber = contasReceberPeriodo.reduce((sum, conta) => sum + conta.valor, 0);
    const totalPagar = contasPagarPeriodo.reduce((sum, conta) => sum + conta.valor, 0);
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text('Relatório Financeiro', 20, 20);
    doc.setFontSize(12);
    ...
    doc.text('Resumo Financeiro', 20, 50);
    doc.setFontSize(10);
    doc.text(`Total a Receber: ${formatBrazilianCurrency(totalReceber)}`, 20, 60);
    doc.text(`Total a Pagar: ${formatBrazilianCurrency(totalPagar)}`, 20, 70);
    doc.text(`Saldo Previsto: ${formatBrazilianCurrency(totalReceber - totalPagar)}`, 20, 80);
    if (contasReceberPeriodo.length > 0) {
      doc.setFontSize(14);
      doc.text('Contas a Receber', 20, 100);
      const receberData = contasReceberPeriodo.map(conta => [
        conta.descricao,
        formatBrazilianCurrency(conta.valor),
        formatBrazilianDate(conta.data_vencimento),
        conta.status === 'recebido' ? 'Recebido' : conta.status === 'vencido' ? 'Vencido' : 'Pendente'
    ...
    if (contasPagarPeriodo.length > 0) {
      const startY = contasReceberPeriodo.length > 0 ? (doc as any).lastAutoTable.finalY + 20 : 110;
      doc.setFontSize(14);
      doc.text('Contas a Pagar', 20, startY);
      const pagarData = contasPagarPeriodo.map(conta => [
        conta.descricao,
        conta.fornecedor,
        formatBrazilianCurrency(conta.valor),
        formatBrazilianDate(conta.data_vencimento),
    ...
    doc.save(`relatorio-financeiro-${format(inicio, 'yyyyMMdd')}-${format(fim, 'yyyyMMdd')}.pdf`);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setIsRelatorioModalOpen(true)} variant="outline">
      ...
      <Dialog open={isRelatorioModalOpen} onOpenChange={setIsRelatorioModalOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsRelatorioModalOpen(false)}>Cancelar</Button>
            <Button onClick={handleGerarRelatorioPDFPeriodo}>
              <FileText className="w-4 h-4 mr-2" />
              Gerar PDF
            </Button>
          </DialogFooter>
