...
// SQL para criar a tabela lembretes
const createLembretesTableSQL = `
...
async function createLembretesTable() {
  try {
    console.log('🔄 Criando tabela lembretes...');
    
    const { data, error } = await supabase.rpc('exec_sql', { 
      sql: createLembretesTableSQL 
    });
    
    if (error) {
      console.error('❌ Erro ao criar tabela lembretes:', error);
      return false;
    }
    
    console.log('✅ Tabela lembretes criada com sucesso!');
    ...
    console.log('✅ Verificação: Tabela lembretes está funcionando!');
    return true;
...
// Executar a função
createLembretesTable().then((success) => {
  if (success) {
    console.log('🎉 Processo concluído com sucesso!');
  } else {
    console.log('❌ Processo falhou. Verifique os erros acima.');
  }
